...
async function processEmails(timestamp, config) {
    return new Promise(async (resolve, reject) => {
        ...
        if (!process.env.IMAP_USER || !process.env.IMAP_PASSWORD) {
            throw new Error('IMAP_USER and IMAP_PASSWORD environment variables must be set');
        }

        let cert = null;
        let key = null;

        if (config.settings.useSelfSignedCerts) {
        ...
        const imapConfig = {
            ...
            tlsOptions: config.settings.useSelfSignedCerts ? {
                ...
                rejectUnauthorized: true // For Gmail, certificate verification is required
            }
        };
                                    ...
                                    simpleParser(stream)
                                        .then(async email => {
                                            ...
                                            if (!attributes.flags.includes('\\Flagged')) {
                                                ...
                                                const emailAnalysis = await analyzeEmail(email.subject, email.from.text, emailBody.substring(0, config.settings.maxEmailChars), email.date, config);
