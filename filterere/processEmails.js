...
async function processEmails(timestamp) {
    return new Promise(async (resolve, reject) => {
        ...
        // Initialize IMAP with TLS configuration (no self-signed certs needed)
        const imapConfig = {
            ...
            tls: true,
            tlsOptions: {
                rejectUnauthorized: false // For Gmail, this is safe
            }
        };
                                    ...
                                    simpleParser(stream)
                                        .then(async email => {
                                            ...
                                            if (!attributes.flags.includes('\\Flagged')) {
                                                ...
                                                const emailBody = (email && email.text) ? email.text : email.html;
